...
static void dma_callback(int lch, u16 ch_status, void *data)
{
    if (channels[lch].owner != NULL) {
        ...
        if (channels[lch].ptransferState->checkPending) {
            __D("  checkPending is set, posting semaphore...\n");
    ...
    switch (cmd & SDMA_IOCCMDMASK) {
      case SDMA_IOCREQUESTDMA:
        ...
        if (result) {
            ...
            sema_init(&(channels[channel].sem), 0);
      ...
      case SDMA_IOCWAITFORCOMPLETION:
        ...
        if (channels[channel].owner == filp) {
            if (down_interruptible(&(channels[channel].sem))) {
...
/*
 *  @(#) ti.sdo.linuxutils.sdma; 1, 0, 0,3; 12-13-2011 18:43:45; /db/atree/library/trees/linuxutils/linuxutils-k02/src/ xlibrary
